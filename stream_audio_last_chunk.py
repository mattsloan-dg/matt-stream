...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        elif "Tail latency measured on is_final" in message or "Tail latency exceeded threshold" in message:
...
async def send_audio(ws, audio_data, chunk_ms, bytes_per_ms, finalize, state, logger):
    ...
    while offset < len(audio_data):
        ...
        # Start timer when last chunk is sent
        if is_last and not state.timer_started.is_set():
            state.timer_start_time = time.perf_counter()
            state.timer_started.set()
            logger.info("Timer started after last chunk sent — measuring tail latency")
...
async def run(file_path, chunk_ms, finalize, dg_params, logger):
    ...
    return {
        "tail_latency_s": state.measured_elapsed,
        "time_to_first_transcript_s": state.first_transcript_time,
    }
...
def parse_repeat(value):
    ...
    if n < 2 or n >= 100:
        raise argparse.ArgumentTypeError(
            f"Repeat count must be >= 2 and < 100, got {n}"
        )
    return n
...
def main():
    ...
    parser.add_argument(
        ...
        type=parse_repeat,
        default=1,
        help="Number of times to repeat the entire run (2-99, default: 1)",
    )
    ...
    for i in range(args.repeat):
        if args.repeat > 1:
            logger.info(f"Run {i + 1} of {args.repeat}")
        result = asyncio.run(run(args.file_path, args.chunk_size, args.finalize, args.dg_params, logger))
        results.append(result)

    if args.repeat > 1:
        tail_latencies = [r["tail_latency_s"] for r in results if r["tail_latency_s"] is not None]
        ttft_values = [r["time_to_first_transcript_s"] for r in results if r["time_to_first_transcript_s"] is not None]
        ...
        lines = [
            "",
            f"{BOLD}{'=' * 60}",
            f"  REPEAT SUMMARY  ({args.repeat} runs)",
            f"{'=' * 60}{RESET}",
        ]

        if tail_latencies:
            avg_tl = sum(tail_latencies) / len(tail_latencies)
            lines += [
                "",
                f"  {CYAN}{BOLD}Tail Latency{RESET}    ({len(tail_latencies)}/{args.repeat} samples)",
                f"  {DIM}{'─' * 40}{RESET}",
                f"    Avg:  {GREEN}{avg_tl:.4f}s{RESET}",
                f"    Min:  {min(tail_latencies):.4f}s",
                f"    Max:  {max(tail_latencies):.4f}s",
            ]
        else:
            lines += [
                "",
                f"  {CYAN}{BOLD}Tail Latency{RESET}    {YELLOW}No samples collected{RESET}",
            ]
        ...
        if ttft_values:
            ...
            lines += [
                "",
                f"  {CYAN}{BOLD}Time to First Transcript{RESET}    ({len(ttft_values)}/{args.repeat} samples)",
                f"  {DIM}{'─' * 40}{RESET}",
                f"    Avg:  {GREEN}{avg_ttft:.4f}s{RESET}",
                f"    Min:  {min(ttft_values):.4f}s",
                f"    Max:  {max(ttft_values):.4f}s",
            ]
        else:
            lines += [
                "",
                f"  {CYAN}{BOLD}Time to First Transcript{RESET}    {YELLOW}No samples collected{RESET}",
            ]
